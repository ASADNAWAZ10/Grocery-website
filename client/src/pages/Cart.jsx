...
const Cart = () => {
    ...
    const {products, currency, cartItem, removefromCart, getCartCount, 
        updateToCart, navigate, getCartAmount, axios, user,setCartItem} = useAppcontext()
    ...
    const placeOrder = async () => {
        try {
            ...
            if(paymentOption === 'COD'){
                ...
                const {data} = await axios.post('/api/order/paypal',{
                    userid: user._id,
                    items: cartArray.map((item)=> ({product: item._id, quantity:item.quantity})),
                    address: selectAddress._id
                })
            ...
            <div className='flex-1 max-w-4xl'>
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div onClick={() => {navigate(`/products/${product.categroy.toLowerCase()}/${product._id}`);  scrollTo(0,0)}}
                            ...
                            <div>
                                ...
                                <div className="font-normal text-gray-500/70">
                                    ...
                                    <div className='flex items-center'>
                                        <p>Qty:</p>
                                        <select onChange={(e) => updateToCart(product._id, Number(e.target.value))}
                                        value={cartItem[product._id]}
                                         className='outline-none'>
                                            {Array(cartItem[product._id] > 9 ? 
                                                cartItem[product._id] : 9
                                            ).fill('').map((_, index) => (
                        ...
                        <button onClick={()=>removefromCart(product._id)} className="cursor-pointer mx-auto">
                           <img src={asset.remove} alt="RemoveIcon"  className="inline-block w-6 h-6"/>
                        </button>
                    </div>)
                )}
                ...
                <button onClick={()=> {navigate('/products'); scrollTo(0,0)}}
                 className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                   <img src={asset.rightArrow} alt="arrow" className=" w-5 h-5 group-hover:-translate-x-1 transition"/>
                    Continue Shopping
                </button>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <button onClick={placeOrder}
                 className="w-full py-3 mt-6 cursor-pointer bg-primary text-white font-medium hover:bg-primary-dull transition">
                    {paymentOption === "USD" ? "Place Order" : "Processed to Checkout"}
